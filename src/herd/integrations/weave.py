...
def behavioral_scores(output: dict, category: str | None = None) -> dict:
    ...
    aliases = {'structure': ('structure', 'syntax'), 'startup': ('startup', 'launch'),
    ...
    return {'available': bool(checks),
            'passed': (all(c['passed'] for c in checks) and not result.get('infrastructure_error')) if checks else None,
            'local_run_id': output.get('run_id'), 'artifact_hash': result.get('artifact_hash'),
...
class WeaveIntegration:
    def __init__(self, project: str | None, state_dir: str | Path):
        ...
        self.client = None
        with self._db() as db:
            db.execute('CREATE TABLE IF NOT EXISTS uploads (key TEXT PRIMARY KEY, kind TEXT, payload TEXT, '
    ...
    def enqueue(self, kind: str, key: str, payload: dict) -> str:
        ...
        with self._db() as db:
            ...
            db.execute('INSERT OR IGNORE INTO uploads(key,kind,payload,status) VALUES (?,?,?,?)',
                       (key, kind, data, 'pending'))
        return key

    def status(self) -> list[dict]:
        with self._db() as db:
            return [dict(row) for row in db.execute('SELECT key,kind,status,remote_ref,error,attempts FROM uploads')]
    ...
    def flush(self) -> dict:
        ...
        for item in self.status():
            ...
            try:
                ...
                payload = {'local_evidence_id': row['key'], 'kind': row['kind'],
                           'evidence': json.loads(row['payload'])}
                ref = weave.publish(sanitize_trace(payload), name=f'herd-evidence-{digest(row["key"])[:24]}')
                with self._db() as db:
                    db.execute("UPDATE uploads SET status='uploaded',remote_ref=?,error=NULL,attempts=attempts+1 WHERE key=?",
                               (ref.uri(), row['key']))
                uploaded += 1
            except Exception as exc:  # noqa: BLE001 - optional SDK outages remain pending
                with self._db() as db:
                    db.execute('UPDATE uploads SET error=?,attempts=attempts+1 WHERE key=?',
    ...
    async def evaluate_attempts(self, name: str, attempts: list[AttemptRecord]) -> dict:
        ...
        for attempt in attempts:
            ...
            key = f'{attempt.task_id}:{attempt.learner_id}'
            if key in group:
        ...
        scorers += [BehaviorScorer(name=category, category=category) for category in
                    ('structure', 'startup', 'semantic_probe', 'interaction_probe', 'integrity')]
        evaluation = weave.Evaluation(name=name, dataset=[{'run_key': key} for key in sorted(common)], scorers=scorers)
        results = {}
        ...
        return {'evaluation_ref': ref.uri() if ref else None, 'mode': 'recorded_oracle_replay',
                'matched_rows': len(common), 'results': results, 'uploaded_at': utcnow()}
    ...
    def publish_leaderboard(self, evaluation_refs: list[str], name: str = 'HERD pool versions') -> str:
        ...
        board = Leaderboard(name=name, description='Recorded behavioral evaluations. Models bind immutable pool hashes.',
                            columns=[LeaderboardColumn(evaluation_object_ref=ref, scorer_name='behavior',
    ...
    async def sync_store(self, store, experiment_id: str, *, upload: bool = True) -> dict:
        ...
        kinds = ('attempt', 'pool', 'gate', 'pair', 'report', 'lesson', 'poisoning_control', 'control_gate', 'control_pair',
                 'curriculum', 'aria_analysis', 'curator_review', 'lifecycle')
        index = {}
        for kind in kinds:
            for value in store.list(experiment_id, kind):
                ...
                identity = next((value[k] for k in ('run_id', 'trial_id', 'pair_id', 'lesson_id',
                                                   'report_hash', 'pool_hash', 'id') if value.get(k)), content_hash)
        ...
        attempts = {a['run_id']: a for a in store.list(experiment_id, 'attempt') if a.get('mode') == 'measured'}
        ...
        for report in store.list(experiment_id, 'report'):
            records = [a for a in attempts.values() if a['task_id'] in final_tasks]
            if records:
        ...
        for local_id, records in groups:
            ...
            if saved and saved.get('content_hash') == binding and saved.get('evaluation_ref'):
                ...
                continue
            try:
                evaluated = await self.evaluate_attempts(f'herd-{digest([experiment_id, local_id])[:24]}',
                ...
                store.put(experiment_id, 'weave_evaluation', local_id,
        ...
        if refs:
            ...
            if not previous or previous.get('content_hash') != binding or not previous.get('remote_ref'):
                try:
                    ...
                    store.put(experiment_id, 'weave_leaderboard', 'current',
...
def sanitize_trace(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {key: sanitize_trace(item) for key, item in value.items()
                if key not in {'self', 'gateway', 'config', 'client', 'messages'}
                and not any(token in key.lower() for token in ('api_key', 'authorization', 'password', 'secret', 'control_token'))}
    if isinstance(value, (list, tuple)):
        return [sanitize_trace(item) for item in value]
    if isinstance(value, str):
        for key, secret in os.environ.items():
            if len(secret) >= 8 and any(token in key.upper() for token in ('API_KEY', 'TOKEN', 'SECRET', 'PASSWORD')):
