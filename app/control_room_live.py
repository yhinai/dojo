...
def _(fetch, mo):
    ...
    _w = mo.callout(err, kind="warn") if err else mo.md("")
    _w
...
def _(attempts, detail, gates, mo, report):
    ...
    _firsts = [a for a in dev_eps if a.get("submission_index") in (None, 1)]
    _succ = sum(1 for a in _firsts if (a.get("result") or {}).get("success"))
    floor = f"{_succ}/{len(_firsts)}" if _firsts else "—"
    cells = [
    ...
    if report and report.get("arms"):
        _ap = (report["arms"].get("admitted_pool") or {}).get("success_rate")
        _np = (report["arms"].get("no_pool") or {}).get("success_rate")
        if _ap is not None and _np is not None:
            cells.append((f"{_ap:.0%} vs {_np:.0%}", "admitted pool vs no pool"))
    mo.Html(
        '<div class="grid">'
...
def _(gates, math, mo, pairs):
    ...
    for g in gates:
        ...
        _dec = str(g.get("decision", "running")).lower()
        _cls = "admit" if _dec in {"admit", "accepted"} else ("reject" if _dec in {"reject", "quarantine"} else "run")
        _pct = max(2, min(100, (math.log(max(_e, 1e-9)) - 0) / (math.log(_thr) - 0) * 100)) if _e > 0 else 2
        _blocks.append(
            f'<div class="gate"><div class="hd"><span class="id">{g.get("trial_id","")[:18]}</span>'
            f'<span class="pill {_cls}">{_dec} · E {_e:.2f}/{_thr:.0f}</span></div>'
            f'<div class="bar {_cls}"><i style="width:{_pct:.0f}%"></i></div>'
    ...
    mo.vstack(
        ...
        + ([mo.Html("".join(_blocks))] if _blocks else [mo.Html('<div class="err">No gates yet.</div>')])
    )
    return
...
def _(dev_eps, mo):
    ...
    _rows = []
    for r in range(1, 4):
        _recs = [a for a in dev_eps if a.get("round_id") == r]
        _firsts = [a for a in _recs if a.get("submission_index") in (None, 1)]
        _succ = sum(1 for a in _firsts if (a.get("result") or {}).get("success"))
        _pct = (_succ / len(_firsts) * 100) if _firsts else 0
        _rows.append(
            f'<div class="gate"><div class="hd"><span class="id">Round {r}</span>'
            f'<span class="pill {"admit" if _firsts else "run"}">{_succ}/{len(_firsts) if _firsts else "—"} first-attempt</span></div>'
            f'<div class="bar {"admit" if _firsts else "run"}"><i style="width:{_pct:.0f}%"></i></div></div>'
        )
    mo.vstack(
...
def _(final_eps, mo, report):
    ...
    _arms = {}
    for a in final_eps:
        _arm = a.get("arm", "unknown")
        _arms.setdefault(_arm, []).append(a)
    _rows2 = []
    for arm in ("no_pool", "curated_docs", "raw_memory", "admitted_pool"):
        _recs = _arms.get(arm, [])
        _succ = sum(1 for a in _recs if (a.get("result") or {}).get("success"))
        _pct = (_succ / len(_recs) * 100) if _recs else 0
        _rows2.append(
            f'<div class="gate"><div class="hd"><span class="id">{arm}</span>'
            f'<span class="pill {"admit" if arm=="admitted_pool" else "run"}">{_succ}/{len(_recs) if _recs else 0}</span></div>'
            f'<div class="bar {"admit" if arm=="admitted_pool" else "run"}"><i style="width:{_pct:.0f}%"></i></div></div>'
        )
    mo.vstack(
...
def _(admitted, lessons, mo):
    ...
    mo.vstack(
        ...
        + ([mo.Html(_rows3)] if admitted else [mo.Html('<div class="err">No admitted lessons yet — a lesson appears only after a gate admits it.</div>')])
    )
    return
