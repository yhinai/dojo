...
Use Linux with systemd, system Python 3.11+, Docker Engine, uv and Caddy 2.8+. Clone the **complete checkout** into `/opt/herd`; runtime bindings depend on source files, the protocol and `uv.lock`, so a standalone wheel is insufficient. Run `sudo sh /opt/herd/deploy/install.sh`. Installation builds the candidate runtime, installs Chromium, locked editable application dependencies, service units and a Caddy environment override. It does not start services or open public ports. The system interpreter prevents a virtualenv from referring to a root-only managed Python path.
