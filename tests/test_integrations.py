...
def test_aria_honest_handoff_and_integrity(tmp_path):
    ...
    record = import_analysis(path, 'Actual report content', 'https://wandb.ai/team/project/aria/report',
                             'operator', 'Add form tasks next development round')
...
def test_trace_redacts_credentials_and_runtime_objects(monkeypatch):
    ...
    output = sanitize_trace({'self': object(), 'api_key': 'private-key-value', 'nested': {
        'source': 'print("private-key-value")', 'messages': [{'secret': 'value'}]}, 'record': object()})
